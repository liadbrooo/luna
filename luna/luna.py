...
class LunaAPI:
    ...
    async def request(
        ...
        async with session.get(url, params=params) as resp:
            if resp.status == 401:
                raise LunaAPIError(401, "Ungültiger oder fehlender API-Key.")
            if resp.status == 403:
    ...
    async def server_status(self) -> Union[dict, list]:
    ...
    async def player_detail(self, player_id: str) -> Union[dict, list]:
        return await self.request("/players/{}".format(player_id))

    async def player_bans(self, player_id: str) -> Union[dict, list]:
        return await self.request("/players/{}/bans".format(player_id))

    async def player_cases(self, player_id: str) -> Union[dict, list]:
        return await self.request("/players/{}/cases".format(player_id))

    async def player_gamedata(
        self, player_id: str, category: str
    ) -> Union[dict, list]:
        return await self.request(
            "/players/{}/gamedata/{}".format(player_id, category)
        )

    async def bans(self, active: Optional[bool] = None) -> Union[dict, list]:
        params = {}
    ...
    async def staff(self) -> Union[dict, list]:
    ...
    async def gamedata(
        ...
        if search:
            params["search"] = search
        return await self.request("/gamedata/{}".format(category), params=params)

    async def gamedata_entry(
        self, category: str, entry_id: str
    ) -> Union[dict, list]:
        return await self.request(
            "/gamedata/{}/{}".format(category, entry_id)
        )

    async def crashes(self) -> Union[dict, list]:
...
class Luna(commands.Cog):
    ...
    async def _get_api(self) -> LunaAPI:
        ...
        if not key:
            raise commands.UserFeedbackCheckFailure(
                ...
                "Nutze `{}lunaset apikey <dein_key>` um ihn zu konfigurieren.".format(
                    inline("[p]")
                )
            )
    ...
    def _embed(self, title: str, color: int = None) -> discord.Embed:
        ...
        return discord.Embed(
            title="{} {}".format(LUNA_EMOJI, title), color=color
        )
    ...
    def _extract_list(data):
        ...
        if isinstance(data, dict):
            items = data.get(
                ...
                data.get(
                    ...
                    data.get(
                        ...
                        data.get(
                            "cases",
                            data.get("crashes", data.get("staff", [])),
                        ),
                    ),
                ),
            )
            total = data.get(
                "total",
                data.get("count", len(items) if isinstance(items, list) else 0),
            )
            return items, total
        return [data], 1
    ...
    def _add_kv_fields(
        ...
        for k, v in d.items():
            ...
            if isinstance(v, dict):
                ...
                val = (
                    humanize_list([str(i) for i in v[:8]]) if v else "\u2014"
                )
                if len(v) > 8:
                    val += " (+{} mehr)".format(len(v) - 8)
            else:
    ...
    async def _send_error(self, ctx: commands.Context, error: LunaAPIError):
        ...
        embed = discord.Embed(
            title="\u274c Luna API Fehler (HTTP {})".format(error.status),
    ...
    async def luna_players_list(
        ...
        **Beispiele:**
        ...
        """
        try:
            data = await (await self._get_api()).players(
        ...
        embed = self._embed("Spielerliste ({})".format(total))
        ...
        for p in items[:25]:
            ...
            pid = _safe_get(
                p, "id", "identifier", "steam", "license", default="\u2014"
            )
            ping = _safe_get(p, "ping")
            line = "**{}** ({})".format(name, pid)
            if ping is not None:
                line += " \u2022 Ping: {}ms".format(ping)
            embed.add_field(name="\u200b", value=line, inline=False)

        if len(items) > 25:
            embed.add_field(
                name="Hinweis",
                value="+ {} weitere Spieler nicht angezeigt.".format(
                    len(items) - 25
                ),
                inline=False,
            )
    ...
    async def luna_players_online(
        ...
        **Beispiele:**
        ...
        try:
            data = await (await self._get_api()).players(
                online=True, search=search, limit=limit
            )
        except LunaAPIError as e:
            return await self._send_error(ctx, e)
        ...
        items, total = self._extract_list(data)
        embed = self._embed(
            "Online Spieler ({})".format(total), color=discord.Color.green()
        )
        ...
        if len(items) > 25:
            embed.add_field(
                name="Hinweis",
                value="+ {} weitere Spieler.".format(len(items) - 25),
                inline=False,
            )
    ...
    async def luna_player_info(
        self, ctx: commands.Context, player_id: str
    ):
        """
        ...
        **Beispiel:**
        ...
        embed = self._embed(
            "Spieler-Details: {}".format(player_id),
        ...
        if isinstance(data, dict):
            ...
            self._add_kv_fields(
                embed, data, skip={"name", "playerName", "username"}
            )
        else:
            embed.description = "```json\n{}\n```".format(data)
    ...
    async def luna_player_bans(
        ...
        **Beispiel:**
        ...
        items, total = self._extract_list(data)
        embed = self._embed(
            "Bans von {} ({})".format(player_id, total),
            color=discord.Color.red(),
        )
        ...
        for i, ban in enumerate(items[:10], 1):
            ...
            expires = _safe_get(
                ban, "expires", "Expires", "expire_date", default="\u2014"
            )
            created = _safe_get(
                ban,
                "created_at",
            ...
            value = (
                ...
                "**Erstellt:** {}\n"
                "**Ablauf:** {}"
            ).format(reason, _bool_emoji(active), banned_by, _fmt_timestamp(created), _fmt_timestamp(expires))
            embed.add_field(name="Ban #{}".format(i), value=value, inline=False)

        if len(items) > 10:
            embed.add_field(
                name="Hinweis",
                value="+ {} weitere Bans.".format(len(items) - 10),
                inline=False,
            )
    ...
    async def luna_player_cases(
        ...
        **Beispiel:**
        ...
        items, total = self._extract_list(data)
        embed = self._embed(
            "Fälle von {} ({})".format(player_id, total),
        ...
        for i, case in enumerate(items[:10], 1):
            ...
            staff = _safe_get(
                case, "staff", "staff_name", "Staff", default="\u2014"
            )
            created = _safe_get(
                case,
                "created_at",
            ...
            value = (
                ...
                "**Staff:** {}\n"
                "**Datum:** {}"
            ).format(ct, reason, staff, _fmt_timestamp(created))
            embed.add_field(name="Fall #{}".format(i), value=value, inline=False)

        if len(items) > 10:
            embed.add_field(
                name="Hinweis",
                value="+ {} weitere Fälle.".format(len(items) - 10),
                inline=False,
            )
    ...
    async def luna_player_gamedata(
        self, ctx: commands.Context, player_id: str, category: str
    ):
        """
        ...
        **Beispiel:**
        ...
        """
        try:
            data = await (await self._get_api()).player_gamedata(
                player_id, category
            )
        except LunaAPIError as e:
            return await self._send_error(ctx, e)

        embed = self._embed("Spieldaten: {} [{}]".format(player_id, category))
        ...
        if isinstance(data, dict):
            ...
            if not data:
                ...
                if len(data) > 10:
                    embed.add_field(
                        name="Hinweis",
                        value="+ {} weitere Einträge.".format(len(data) - 10),
                        inline=False,
                    )
        else:
            embed.description = "```json\n{}\n```".format(data)
    ...
    async def luna_bans(
        self, ctx: commands.Context, active_only: bool = True
    ):
        """
        Zeigt alle Bans an.
        ...
        **Beispiel:**
        ...
        embed = self._embed(
            "{} ({})".format(label, total), color=discord.Color.red()
        )
        ...
        for i, ban in enumerate(items[:10], 1):
            ...
            pid = _safe_get(
                ban, "identifier", "id", "steam", default="\u2014"
            )
            ...
            value = (
                ...
                "**Von:** {}\n"
                "**Ablauf:** {}"
            ).format(player, pid, reason, _bool_emoji(active), banned_by, _fmt_timestamp(expires))
            embed.add_field(name="Ban #{}".format(i), value=value, inline=False)
    ...
    async def luna_cases(
        self, ctx: commands.Context, case_type: Optional[str] = None
    ):
        """
        ...
        **Beispiele:**
        ...
        label = "Fälle"
        if case_type:
            label += " (Typ: {})".format(case_type)
        embed = self._embed(
            "{} ({})".format(label, total), color=discord.Color.orange()
        )
        ...
        for i, case in enumerate(items[:10], 1):
            ...
            reason = _safe_get(case, "reason", "Reason", default="\u2014")
            staff = _safe_get(
                case, "staff", "staff_name", default="\u2014"
            )
            created = _safe_get(
                case, "created_at", "createdAt", default="\u2014"
            )
            ...
            value = (
                ...
                "**Staff:** {}\n"
                "**Datum:** {}"
            ).format(ct, player, reason, staff, _fmt_timestamp(created))
            embed.add_field(name="Fall #{}".format(i), value=value, inline=False)
    ...
    async def luna_staff(self, ctx: commands.Context):
        ...
        for i, member in enumerate(items[:25], 1):
            ...
            role = _safe_get(
                member, "role", "rank", "Role", default="\u2014"
            )
            identifier = _safe_get(
                member, "identifier", "steam", "id", default="\u2014"
            )
            value = "**Rolle:** {}\n**Identifier:** {}".format(role, identifier)
            embed.add_field(
                name="{}. {}".format(i, name), value=value, inline=False
            )

        if len(items) > 25:
            embed.add_field(
                name="Hinweis",
                value="+ {} weitere Mitglieder.".format(len(items) - 25),
                inline=False,
            )
    ...
    async def luna_gamedata_list(
        ...
        **Beispiele:**
        ...
        """
        try:
            data = await (await self._get_api()).gamedata(
                category, search=search, limit=limit
            )
        except LunaAPIError as e:
            return await self._send_error(ctx, e)

        items, total = self._extract_list(data)
        embed = self._embed("Game-Data: {} ({})".format(category, total))
        ...
        for i, item in enumerate(items[:15], 1):
            ...
            embed.add_field(
                name="#{}".format(i), value=lines or "\u2014", inline=False
            )

        if len(items) > 15:
            embed.add_field(
                name="Hinweis",
                value="+ {} weitere Einträge.".format(len(items) - 15),
                inline=False,
            )
    ...
    async def luna_gamedata_entry(
        self, ctx: commands.Context, category: str, entry_id: str
    ):
        """
        ...
        **Beispiel:**
        ...
        """
        try:
            data = await (await self._get_api()).gamedata_entry(
                category, entry_id
            )
        except LunaAPIError as e:
            return await self._send_error(ctx, e)

        embed = self._embed("Game-Data: {} #{}".format(category, entry_id))
    ...
    async def luna_crashes(self, ctx: commands.Context):
        ...
        for i, crash in enumerate(items[:10], 1):
            player = _safe_get(
                crash, "player_name", "name", default="\u2014"
            )
            reason = _safe_get(
                crash, "reason", "Reason", default="\u2014"
            )
            module = _safe_get(
                crash, "module", "Module", default="\u2014"
            )
            created = _safe_get(
                crash, "created_at", "createdAt", default="\u2014"
            )
            ...
            value = (
                ...
                "**Modul:** {}\n"
                "**Zeitpunkt:** {}"
            ).format(player, reason, module, _fmt_timestamp(created))
            embed.add_field(
                name="Crash #{}".format(i), value=value, inline=False
            )

        if len(items) > 10:
            embed.add_field(
                name="Hinweis",
                value="+ {} weitere Reports.".format(len(items) - 10),
                inline=False,
            )
...
class LunaSet(commands.Cog):
    ...
    async def lunaset_apikey(
        self, ctx: commands.Context, *, api_key: str
    ):
        """
        ...
        **Beispiel:**
        ...
        try:
            ...
            status_msg = "\u26a0\ufe0f **Verbindungstest fehlgeschlagen:** {}".format(
                e.message
            )

        embed = discord.Embed(
            title="\u2699\ufe0f Luna API-Key gespeichert",
            description="{}\n\nDer Key wurde sicher in der Bot-Konfiguration gespeichert.".format(
                status_msg
            ),
    ...
    async def lunaset_test(self, ctx: commands.Context):
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description="Die Luna API ist erreichbar und der Key ist gültig.",
                color=discord.Color.green(),
            )
            if isinstance(data, dict):
            ...
            embed = discord.Embed(
                ...
                description="HTTP {}: {}".format(e.status, e.message),
                color=discord.Color.red(),
            )
